...
{   
  if(argc != 6) {
    printf("Usage: ./main calib_result.txt input_image output_image [zoom, default:4] [vis = 0?1]\n");
    return -1;
  }
  char *calib = argv[1];
  char *input = argv[2];
...
//  printf("m_row= %2.4f, m_col=%2.4f\n", point2D[0], point2D[1]);
  ...
  /* --------------------------------------------------------------------*/  
  IplImage *src1         = cvLoadImage(input);      // source image 1
